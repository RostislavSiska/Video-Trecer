...
class Movie(Base):
    ...
    release_date: Mapped[Optional[str]]  # можно хранить как строку, потом преобразовывать
    vote_average: Mapped[Optional[float]] = mapped_column(Float)
    runtime: Mapped[Optional[int]]  # минуты
    production_countries: Mapped[Optional[List[str]]] = mapped_column(ARRAY(String))

    # Связь с оценками: один фильм -> много оценок
